...
public class HexAI {
     ...
     * Nếu cần hard-timeout, có thể thêm kiểm tra System.currentTimeMillis() tại
     * đây.
     *
     ...
     * - Khi depth == 0: đánh giá bằng hàm heuristic (khoảng cách đường đi ngắn
     * nhất)
     *
    ...
    private int minimax(HexGame state, boolean maximizing, int depth, int alpha, int beta) {
        ...
        if (winner == HexGame.BLUE)
            return WIN + depth;
        if (winner == HexGame.RED)
            return -WIN - depth;
        if (depth == 0)
            return heuristic(state);
    ...
    /**
     * UC-04 – Tính đường đi ngắn nhất từ cạnh xuất phát đến cạnh đích của người
     * chơi.
